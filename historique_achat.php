...
<div class="col-md-12">
    ...
    if(isset($_GET['startdate']) AND isset($_GET['finishtdate'])){
        ...
        $q = "SELECT * from achat where date(date) BETWEEN '$startdate' AND '$finishdate' ORDER by date desc";
        addToLog($q);
        echo'<h1 align="center">Historique des achat entre '.$startdate.' et '.$finishdate.'</h1><br>';
    }elseif(isset($_GET['article'])){
        $id=$_GET['article'];
        $q = "SELECT * from achat where article_id=$id ORDER by date desc";
        echo'<h1 align="center">Historique des achats de:<h1><h3>' .getArticleInfo($id)['designiation']. '</h3><br>';
    }else {
        $todaydate= date('Y/m/d',mktime(0, 0, 0, date("m")  , date("d"), date("Y")));
        $q = "SELECT * from achat where date(date)='$todaydate' ORDER by date desc";
        addToLog($q);
        echo'<h1 align="center">Historique des achats <?= $todaydate ?></h1><br>';
    }
    ?>
    ...
    <div class="col-md-12">
        ...
        <form action="historique_achat.php" method="get">
    ...
    <table class="table table-hover" style="table-layout:fixed;" id="articleTable">
        ...
        while($historique = mysqli_fetch_assoc($r)){
            ...
            <tr>
                ...
                <td><?= $historique['qte_achat']*$historique['prix_achat_fournisseur'] ?></td>
                <td><?= $historique['date'] ?></td>
                <td><?= getFournisseurInfo($historique['fournisseur_id'])['f_nom'] ?></td>
                <td><?= $historique['facture_achat_id'] ?></td>
            </tr>
