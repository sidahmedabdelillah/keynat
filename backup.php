...
$q="UPDATE config set c_val=(SELECT round(count(*)/2) FROM `article` WHERE quantite>0) where c_name='devision'";
$r=@mysqli_query($dbc,$q);
if($r){
  echo("Calcul de devision: ok</br>");
}

$filename = 'hanout.sql';
if(file_exists($filename)){
  @unlink('hanout.sql');
  echo("Supprission du fichier: ok</br>");
}else{
 ...
 */
define("DB_USER", 'root');
define("DB_PASSWORD", 'SUgrfF4tPPtywWNN');
define("DB_NAME", 'keynat84_hanout');
define("DB_HOST", 'localhost');
define("BACKUP_DIR", '.'); // Comment this line to use same script's directory ('.')
//define("TABLES", '*'); // Full backup
define("TABLES", 'article,article_maintenance,maintenance,pc_nadir,achat,client'); // Partial backup
define("CHARSET", 'utf8');
...
$file = 'hanout.sql';
$remote_file = 'public_html/hanout.sql';

// Mise en place d'une connexion basique
$conn_id = ftp_connect('keynatech.com');

// Identification avec un nom d'utilisateur et un mot de passe
$login_result = ftp_login($conn_id, 'keynat84', 'txRI5HARt7eK');
...
<script type="text/javascript">
  window.location = "http://keynatech.com/restore.php";
</script>
